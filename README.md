...
- **Changelog module (optional, off by default)**: if your plugin maintains a single changelog file that gets new lines appended per commit (like mcMMO's `Changelog.txt`), the latest-build card can show what's new since the previous build. It's gated behind `CHANGELOG_ENABLED` in the script config block — leave it `false` and the feature is fully inert (`getChangelog()` always returns `[]`). To use it: (1) set `CHANGELOG_ENABLED = true`, (2) add a build workflow step that diffs your changelog file between the previous build's commit and the new one and writes the added lines into a `**Changelog:**` section in the release body (right before the `**Verify the JAR:**` section — that's the boundary the site's regex looks for). See [mcmmo-builds/.github/workflows/build.yml](https://github.com/dubsector/mcmmo-builds/blob/main/.github/workflows/build.yml) (the "Generate release notes" step) for the full reference implementation, including filtering out version-bump headings as structural noise. The card shows the first 4 items with a "Show N more" / "Hide" toggle rather than an unbounded list, so a big multi-commit changelog doesn't push the download button below the fold — don't replace this with a scrollable inner box, it's worse on mobile.
